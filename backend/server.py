...
@api_router.get("/partisipasi/admin-list", response_model=List[PartisipasiResponse])
async def get_admin_partisipasi_list():
    projection = {
        "_id": 0,
        "id": 1,
        "email": 1,
    ...
    partisipasi_list = await db.partisipasi.find({}, projection).sort("created_at", -1).to_list(10000)
